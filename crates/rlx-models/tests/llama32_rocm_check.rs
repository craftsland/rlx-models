...
mod rocm_tests {
    ...
    /// The rocm result must equal CPU's, not merely be finite.
    ///
    ...
    #[test]
    fn llama32_tiny_graph_matches_cpu_on_rocm() {
        if !rlx_runtime::is_available(Device::Rocm) {
            eprintln!("skip: ROCm not available");
            return;
        }
        let cpu = run(Device::Cpu);
        let dev = run(Device::Rocm);
        super::compile_support::assert_matches_cpu("rocm", &cpu, &dev, 0.002);
    }
}
